...
const superCar = new CarFactory('Honda', 'Pilot')
...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  Movie.prototype.changeRating = function (number){
    let updatedRating = this.rating + number / 2
  ...
  //name, age, email, and savedPosts in that order. 
  //Name and email will be strings, 
  //age will be a number and 

  //savedPosts will be an array of objects. 
  
  //These objects will each have 
  //3 properties: id (a number), title (a string), 
  //and rating (a number between 1 and 5). 
  //These objects are the posts that the user will have saved to their account.
  
  //Once the User constructor function is created, write a 
  //prototype method 
  //for the User function. Name this method addSavedPost. 
  //It should take in three parameters: id (a number), 
  //title (a string) and rating (a number). 
  ...
  User.prototype.addSavedPost = function (id, title, rating){
    const newPost = {id, title, rating}
    this.savedPosts.push(newPost)
    console.log(this)
  }
  
//Below not needed reference for future. 
  let objArray = [{id: 5, title: '2021 Motorcycle Trends', rating: 3}]
  const savedPosts = new User('Nick M', 40, 'nick@gmail.com', objArray)
  savedPosts.addSavedPost(4, 'Awesome News Post', 5)
  ...
  //that will take in one number parameter representing the post id. Use this id to find 
  //and remove the matching object in the savedPosts array.
  ...
  //  Once you find the matching object, update
  //   it's rating score with the new rating parameter.
